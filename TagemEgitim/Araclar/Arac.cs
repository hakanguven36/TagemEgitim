...
namespace TagemEgitim.Araclar
{
	...
	public class Pagination<T>
	{
		...
		public List<T> GetPage(List<T> liste, int pageNo, int indexofSelected)
		{
			itemCount = liste.Count();
			totalPage = Convert.ToInt32( Math.Ceiling(itemCount / (pageSize * 1.0)));
			currentPage = pageNo;
			if(indexofSelected > 0)
			{
				currentPage = Convert.ToInt32(Math.Ceiling((indexofSelected +1) / (pageSize * 1.0)));
			}
